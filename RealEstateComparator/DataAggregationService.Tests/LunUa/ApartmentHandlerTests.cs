...
{
    public class LunUaApartmentHandler
    {
        [Fact]
        ...
        private Mock<PageHandler> MockPageHandler(Mock<HtmlParser> htmlParserMock, IEnumerable<Apartment> expectedResult, IEnumerable<ApartComplex> apartComplexes)
        {
            ...
            MockParseHtmlNumOfRooms(pageHandlerMock, htmlApartmentNodes, expectedResult);
            MockHasMultipleFloors(pageHandlerMock, htmlApartmentNodes, expectedResult);
            MockParseHtmlRoomSpace(pageHandlerMock, htmlApartmentNodes, expectedResult);
            MockParseHtmlApartPrice(pageHandlerMock, htmlApartmentNodes, expectedResult);

            return pageHandlerMock;
        }

        private void MockLoadApartmentsHtml(Mock<PageHandler> pageHandlerMock, IEnumerable<HtmlNode> htmlApartmentNodes, IEnumerable<ApartComplex> apartComplexes)
        {
            pageHandlerMock
                .Setup(pageHandler => pageHandler.LoadApartmentsHtml(apartComplexes.ToList()[0].Url))
                .ReturnsAsync(new HtmlNodeCollection(null)
                {
                    htmlApartmentNodes.ToList()[0],
                    htmlApartmentNodes.ToList()[1]
                });
            ...
            pageHandlerMock
                ...
                .ReturnsAsync(new HtmlNodeCollection(null)
                {
                    htmlApartmentNodes.ToList()[2],
                    htmlApartmentNodes.ToList()[3]
                });
        }
        ...
        private void MockParseHtmlRoomSpace(Mock<PageHandler> pageHandlerMock, IEnumerable<HtmlNode> htmlApartmentNodesInput, IEnumerable<Apartment> expectedResultInput)
        {
            ...
            {
                pageHandlerMock
                    .Setup(pageHandler => pageHandler.ParseHtmlRoomSpace(htmlApartmentNodes[iter]))
                    .Returns(new Tuple<int, int>(expectedResult[iter].DwellingSpaceMin, expectedResult[iter].DwellingSpaceMax));
            }
        }
        ...
        private void MockParseHtmlApartPrice(Mock<PageHandler> pageHandlerMock, IEnumerable<HtmlNode> htmlApartmentNodesInput, IEnumerable<Apartment> expectedResultInput)
        {
            ...
            {
                pageHandlerMock
                    .Setup(pageHandler => pageHandler.ParseHtmlApartPrice(htmlApartmentNodes[iter]))
                    .Returns(new Tuple<int, int>(expectedResult[iter].SquareMeterPriceMin, expectedResult[iter].SquareMeterPriceMax));
            }
        }
        ...
        private IEnumerable<ApartComplex> CreateApartComplexes()
        {
            ...
            {
                new ApartComplex(){CityName = "Київ", Name = "ЖК Лебединий", Source = "LunUa", Url = "https://lun.ua/uk/жк-лебединий-київ"},
                new ApartComplex(){CityName = "Київ", Name = "ЖК Chalet", Source = "LunUa", Url = "https://lun.ua/uk/жк-chalet-київ"}
            };
        }
